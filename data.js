...
// Course Matrix defining subjects and target weekly hours per grade level
// Low (1-2) = 23 periods, Mid (3-4) = 29 periods, High (5-6) = 32 periods
export const SUBJECTS_MATRIX = {
  // Low Grades (1, 2)
  low: [
    { subject: "國語", weeklyHours: 6, requiresRoom: null },
    { subject: "數學", weeklyHours: 3, requiresRoom: null },
    { subject: "生活", weeklyHours: 6, requiresRoom: null }, // 生活包含美勞、音樂等
    { subject: "綜合", weeklyHours: 2, requiresRoom: null },
    { subject: "健康", weeklyHours: 1, requiresRoom: null },
    { subject: "體育", weeklyHours: 2, requiresRoom: "體育" },
    { subject: "閱讀", weeklyHours: 1, requiresRoom: null },
    { subject: "彈性", weeklyHours: 2, requiresRoom: null }
  ],
  // Mid Grades (3, 4)
  mid: [
    { subject: "國語", weeklyHours: 5, requiresRoom: null },
    { subject: "數學", weeklyHours: 4, requiresRoom: null },
    { subject: "社會", weeklyHours: 3, requiresRoom: null },
    { subject: "自然", weeklyHours: 3, requiresRoom: null },
    { subject: "音樂", weeklyHours: 2, requiresRoom: "音樂" },
    { subject: "美勞", weeklyHours: 2, requiresRoom: "美勞" },
    { subject: "綜合", weeklyHours: 2, requiresRoom: null },
    { subject: "健康", weeklyHours: 1, requiresRoom: null },
    { subject: "體育", weeklyHours: 2, requiresRoom: "體育" },
    { subject: "電腦", weeklyHours: 1, requiresRoom: "電腦" },
    { subject: "英語", weeklyHours: 2, requiresRoom: null },
    { subject: "彈性", weeklyHours: 2, requiresRoom: null }
  ],
  // High Grades (5, 6)
  high: [
    ...
    { subject: "健康", weeklyHours: 1, requiresRoom: null },
    { subject: "體育", weeklyHours: 2, requiresRoom: "體育" },
    { subject: "電腦", weeklyHours: 2, requiresRoom: "電腦" },
    { subject: "英語", weeklyHours: 3, requiresRoom: null },
    { subject: "彈性", weeklyHours: 3, requiresRoom: null }
  ]
};
...
export function parseTeacherCSV(csvText) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Support standard CSV split (handling optional quotes if any, simple regex split)
    ...
    const roleStr = row[2] || '鐘點';
    const role = roleMap[roleStr] || 'hourly';
    
    // If baseHours is omitted, auto fill defaults based on role
    const baseHours = row[3] ? parseInt(row[3]) : defaultHours[role];
    const targetClassId = row[4] || null;
    const specialties = row[5] ? row[5].split(';').map(s => s.trim()).filter(Boolean) : [];
    ...
    teachers.push({
      ...
      busySlots: [] // Initial default is no busy slots, admins can edit in UI
    });
  }
  return teachers;
}
 ...
 */
export function validateAssignments(teachers, assignments) {
  // Reset counts
  ...
  // Underloaded and overloaded logic (hourly teachers have 0 base hours, so they are excluded)
...
export function getInitialState() {
  ...
  return {
    ...
    assignments: assignments, // Initially pre-configured with empty teachers
    schedule: null   // Initially empty, will hold { classId: { "day-period": { subject, teacherId, requiresRoom } } }
  };
}
